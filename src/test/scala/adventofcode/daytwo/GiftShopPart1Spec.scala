...
class GiftShopPart1Spec extends AnyWordSpec with Matchers {
        ...
        maybeRanges match {
          case Success(ranges) =>
            val sillyIds = ranges.head.map { range =>
              SillyIdFinder.findSillyIdsPart1(range)
            }

            sillyIds should have size 11
            sillyIds(0) should be(Set(BigDecimal(11), BigDecimal(22)))
            sillyIds(1) should be(Set(BigDecimal(99)))
            sillyIds(2) should be(Set(BigDecimal(1010)))
            ...
            sillyIds(8) should be(Set.empty)
            sillyIds(9) should be(Set.empty)
            sillyIds(10) should be(Set.empty)
          case _ => fail("Failed to parse ranges")
        }
      }
      ...
      "calculate the correct sum of all silly IDs" in new Fixture {
        ...
        maybeRanges match {
          case Success(ranges) =>
            ...
            // Sum: 11 + 22 + 99 + 1010 + 1188511885 + 222222 + 446446 + 38593859
            sumOfSillyIds should be(BigDecimal("1227775554"))
          case _ => fail("Failed to parse ranges")
        }
      }
      ...
      "process all ranges correctly from start to finish" in new Fixture {
        ...
        maybeRanges match {
          case Success(ranges) =>
            ...
            results.map(_._3).sum should be(BigDecimal("1227775554"))
          case _ => fail("Failed to parse ranges")
        }
      }
    }
      ...
      "process range 95-115 correctly" in new Fixture {
        ...
        maybeRanges match {
          case Success(ranges) =>
            ...
            sillyIds should be(Set(BigDecimal(99)))
            sillyIds.sum should be(BigDecimal(99))
          case _ => fail("Failed to parse ranges")
        }
      }
